...
describe('GET /api/v1/integrations/ga4/status', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns disconnected status when no connection exists', async () => {
    ...
    expect(json.provider).toBe('analytics')
    expect(json.propertyId).toBeNull()
  })

  it('returns connected status with property ID', async () => {
    mockSupabase.maybeSingle.mockResolvedValueOnce({
      data: {
        id: 'conn-1',
        provider: 'analytics',
        site_url: 'properties/123456',
        status: 'active',
