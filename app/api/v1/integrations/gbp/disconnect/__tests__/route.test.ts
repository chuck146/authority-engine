...
describe('POST /api/v1/integrations/gbp/disconnect', () => {
  beforeEach(() => {
  ...
  it('still deletes row even if token revocation fails', async () => {
    const { revokeToken } = await import('@/lib/google/oauth')
    vi.mocked(revokeToken).mockRejectedValueOnce(new Error('Revocation failed'))
